const { BOT_TOKEN, OMS_SERVER } = process.env;
...
module.exports.Telegraf = class Telegraf extends EventEmitter {
    constructor() {
        ...
        this.bot = new TelegrafOrg(process.env.BOT_TOKEN);
        ...
        this.bot.command('register', async (ctx) => {
            ...
            try {
                let res = await this.FDataNode.post(
                    OMS_SERVER + "/pgapi/OpenCommunitySecurity/Account",
                    ctx.update.message
                );
        ...
        this.bot.hears('EMERGENCIA', async (ctx) => {
            let message = "";
            try {
        ...
        this.bot.hears('ADVERTENCIA', async (ctx) => {
            let message = "";
            try {
        ...
        this.bot.hears('PROBAR', async (ctx) => {
            let message = "";
            try {
        ...
        this.bot.hears('APAGAR', async (ctx) => {
            let message = "";
            try {
    ...
    static async SendEvent(ctx, eventcode) {
        ...
        if (ctx.message) {
            data = { idgroup: Telegraf.getidgroup(ctx.message.chat.id), message: ctx.message, eventcode: eventcode };
        } else {
            data = { idgroup: ctx.idgroup, message: ctx, eventcode: eventcode };
        }
        */
        ...
        try {
            ...
            let res = await FDataNodeFetch.post(
                OMS_SERVER + "/pgapi/OpenCommunitySecurity/event",
                data
            );
            let data_res = await res.json();
    ...
    static async EmitEventToNameSpace(socketioInstance, pgNotify) {
        ...
        //1) identificar el canal de notificación
     ...
     CreateSocketIONameSapce(socketioInstance) {
        ...
        namespace_appocs.on("connection", (socket) => {
            ...
            socket.on("ocsdevconnected", (socket1) => {
                try {
                    //await Telegraf.SendEvent(ctx, 'ocsdevconnected');
