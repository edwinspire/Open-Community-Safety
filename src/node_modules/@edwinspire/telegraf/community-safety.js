...
//        console.log('>>>>>>> event.idgroup', event.details.message.from);
        if (event && event.idevent > 0) {
            if (typeof event.identification !== 'undefined' && event.notify) {
                let message = event.details.message.from.first_name + ' ' + event.details.message.from.last_name + ' envía ' + event.label;
                this.bot.telegram.sendMessage(
                    event.identification,
                    message);
            }
        } else {
            console.trace('sendMessageToGroup evento no válido', event);
        }
    }

    async ProcessPgNotification(socketio, pgNotifyEvent) {

        console.log('===>>>>   pgNotifyEvent.channel', pgNotifyEvent);
