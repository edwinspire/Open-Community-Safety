...
export class WebSocketExpress extends EventEmitter {
  constructor(httpServer, root_path, async_authentication_callback) {
    super()
    this.root_path = root_path || '/ws'
    this.authentication = async_authentication_callback
    this.paths = []
      ...
      try {
        if (urlData.pathname.startsWith(this.root_path)) {
          if (this.authentication) {
            let aut = await this.authentication(request, socket, head, urlData)
            if (aut && aut.isAuthenticated) {
